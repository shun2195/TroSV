		...
		<div class="row g-4">
				...
				<div class="card sticky-top shadow-sm" style="top: 80px;">
					...
					<div class="card-body">
						<form action="${pageContext.request.contextPath}/danh-sach-phong"
							...
							<div class="mb-3">
								<label for="giaSelect" class="form-label fw-semibold">Khoảng
									giá</label> <select class="form-select" name="khoangGia" id="giaSelect">
			...
			<div class="col-lg-9">
				<div class="d-flex align-items-center mb-3 p-3 bg-light rounded">
					<h4 class="mb-0">
						Tìm thấy <span class="text-primary fw-bold">${dsPhongTro.size()}</span>
						phòng trọ phù hợp
					</h4>
				</div>
					...
					<c:forEach items="${dsPhongTro}" var="phong">
						<div class="col">
							<div class="card h-100 border-0 shadow-sm room-card">
								...
								<c:choose>
									<c:when
										test="${not empty phong.hinhanh.duongDan and not fn:startsWith(phong.hinhanh.duongDan, 'http')}">
										<img
											src="${pageContext.request.contextPath}/${phong.hinhanh.duongDan}"
											class="card-img-top"
											style="height: 200px; object-fit: cover;"
											alt="${phong.tieuDe}">
											<p style="font-size: 10px; color: red;">DEBUG: ${pageContext.request.contextPath}/${phong.hinhanh.duongDan}</p>
									</c:when>
									<c:otherwise>
										<img src="${phong.hinhanh.duongDan}" class="card-img-top"
											style="height: 200px; object-fit: cover;"
											alt="${phong.tieuDe}">
									</c:otherwise>
								</c:choose>
								...
								<div class="card-body d-flex flex-column">
									...
									<p class="card-text text-body-secondary small mb-2">
										<i class="bi bi-geo-alt-fill me-1"></i> ${phong.diaChi},
					...
					<c:if test="${empty dsPhongTro}">
						<div class="col-12">
							<div class="text-center p-5 border rounded bg-light">
								<i class="bi bi-binoculars-fill text-muted"
									style="font-size: 4rem;"></i>
								<h4 class="mt-3">Không tìm thấy kết quả</h4>
								<p class="lead text-body-secondary">Rất tiếc, không có phòng
									trọ nào phù hợp với bộ lọc của bạn.</p>
							</div>
						</div>
					</c:if>
				</div>
